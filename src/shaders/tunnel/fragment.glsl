...
float metaball(vec3 p){
    float fv[5];
    float tt = t * 0.01;
    fv[0] = length(p - vec3(2.0 * sin(tt), 0.0, 2.0 * cos(tt)));
    fv[1] = length(p - vec3(4.0 * sin(tt), sin(tt), 2.0 * cos(tt * 0.7)));
    fv[2] = length(p - vec3(1.0 * sin(tt), 2.0 * cos(tt * 1.3), 1.0 * cos(tt)));
    fv[3] = length(p - vec3(4.0 * cos(tt), 2.0 * cos(tt * 1.3), 1.5 * cos(tt)));
    fv[4] = length(p - vec3(0.5 * sin(tt * 0.2), 2.0 * cos(tt * 1.6), 0.5 * sin(tt)));
    float len = 0.0;
    float fs = 1.0;
    for (int i = 0; i < 5; i ++) {
        len += fs / (fv[i] * fv[i]);
...
void main(void)
{
    ...
    float d;
    for (int i = 0; i < 64; i++) {
        d = metaball(P.xyz * (11. - 10. * tunnelAmount));
        P = P + d * dir;
    }
